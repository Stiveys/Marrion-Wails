...
function isSpam(data: Record<string, unknown>) {
  const honeypot = typeof data.website === 'string' && data.website.trim().length > 0;
  const msg = sanitize((data.additionalInfo ?? data.message) as string, 2000);
  const links = (msg.match(/https?:\/\/|www\./gi) || []).length;
  const repeated = /(.)\1{6,}/.test(msg);
  return honeypot || links > 4 || repeated;
}
...
export default async function handler(req: Request): Promise<Response> {
  ...
  const phone = sanitize(data.phone, 100);
  const insuranceType = sanitize(data.insuranceType, 100);
  const coverageAmount = sanitize(data.coverageAmount, 100);
  const timestamp = new Date().toISOString();

  const subject =
    `New Quote Submission - ${firstName} ${lastName}` +
  ...
  const html = `
    <div style="font-family: -apple-system, Segoe UI, Roboto, Arial; color: #111;">
      ...
      <ul style="list-style:none;padding:0;margin:0 0 16px;">
        ...
        <li><strong>Coverage:</strong> ${coverageAmount}</li>
      </ul>
      <h3 style="margin:20px 0 8px;">Specific Details</h3>
      <ul style="list-style:none;padding:0;margin:0 0 16px;">
        <li><strong>Vehicle:</strong> ${sanitize(data.vehicleMake, 100)} ${sanitize(data.vehicleModel, 100)} ${sanitize(data.vehicleYear, 10)} | Value: ${sanitize(data.vehicleValue, 100)}</li>
        <li><strong>Property:</strong> ${sanitize(data.propertyType, 100)} | Value: ${sanitize(data.propertyValue, 100)}</li>
      ...
      <pre style="white-space: pre-wrap; background:#f6f8fa; padding:12px; border-radius:8px; color:#333;">${sanitize((data.additionalInfo ?? data.message) as string, 2000)}</pre>
      <p style="margin:16px 0;"><strong>Preferred Contact:</strong> ${sanitize(data.preferredContact, 50)} | <strong>Urgency:</strong> ${sanitize(data.urgency, 50)}</p>
  ...
  try {
    ...
    if (webhook) {
      fetch(webhook, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ ...log, payload: { ...data, email } }),
      }).catch(() => {});
    }
